package com.profilemaker.dpgenerator.photoprofilepic.photoeditor.model;

import java.util.ArrayList;
import java.util.List;

public class ColorCode {
    public static List<String>colorList() {
        List<String>arrayList = new ArrayList<>();
        //FREE
        arrayList.add("#FFFFFF");
        arrayList.add("#BBBBBB");
        arrayList.add("#4E4E4E");
        arrayList.add("#212121");
        arrayList.add("#000000");
        arrayList.add("#FFD7CD");
        arrayList.add("#F9AB9D");
        arrayList.add("#EC5C60");
        arrayList.add("#CB3243");
        arrayList.add("#CD181F");
        arrayList.add("#FF0000");
        arrayList.add("#FFF2CA");
        arrayList.add("#FDE472");
        arrayList.add("#F3AF59");
        arrayList.add("#FC7F3D");
        arrayList.add("#ED3F0F");
        arrayList.add("#FFF1F1");
        arrayList.add("#FFE1E4");
        arrayList.add("#FFA4B9");
        arrayList.add("#FF679F");
        arrayList.add("#FB2C78");
        arrayList.add("#E7D5E7");
        arrayList.add("#D3A6D8");
        arrayList.add("#BA66AF");
        arrayList.add("#A53B8E");
        arrayList.add("#65218C");
        arrayList.add("#99D2F9");
        arrayList.add("#81ADEA");
        arrayList.add("#2961A9");
        arrayList.add("#0E2E89");
        arrayList.add("#171982");
        arrayList.add("#A5E7F6");
        arrayList.add("#7CE3FF");
        arrayList.add("#00B0D0");
        arrayList.add("#058BC0");
        arrayList.add("#08447E");
        arrayList.add("#DEEEE9");
        arrayList.add("#B3D0C5");
        arrayList.add("#4DAF9D");
        arrayList.add("#21887C");
        arrayList.add("#0F664E");
        arrayList.add("#D3E5A6");
        arrayList.add("#AACE87");
        arrayList.add("#A3AF38");
        arrayList.add("#6D822B");
        arrayList.add("#366131");
        arrayList.add("#E4D9C0");
        arrayList.add("#D6C392");
        arrayList.add("#A3815A");
        arrayList.add("#72462F");
        arrayList.add("#3E3129");
        //PRO 1
        arrayList.add("#A7A7A7");
        arrayList.add("#995452");
        arrayList.add("#E1D6D7");
        arrayList.add("#CCBFD4");
        arrayList.add("#A67C80");
        arrayList.add("#D4D3CF");
        arrayList.add("#7A7185");
        arrayList.add("#6E5055");
        arrayList.add("#666563");
        arrayList.add("#F0E4FB");
        arrayList.add("#EED0D5");
        arrayList.add("#E1CCD1");
        arrayList.add("#C3C9DF");
        arrayList.add("#929493");
        arrayList.add("#807D5C");
        arrayList.add("#E2E4E1");
        arrayList.add("#8596A4");
        arrayList.add("#ECECEC");
        arrayList.add("#96A48C");
        arrayList.add("#AFB0B2");
        arrayList.add("#BEC0BF");
        arrayList.add("#B7C5B3");
        arrayList.add("#A0A7BA");
        arrayList.add("#FFFAF7");
        arrayList.add("#7D8970");
        //PRO 2
        arrayList.add("#FF7063");
        arrayList.add("#FF3274");
        arrayList.add("#3EA2D7");
        arrayList.add("#FFA258");
        arrayList.add("#F22D52");
        arrayList.add("#F2A0B6");
        arrayList.add("#7285F2");
        arrayList.add("#3ADB7C");
        arrayList.add("#F2C4CD");
        arrayList.add("#129AEF");
        arrayList.add("#0278A6");
        arrayList.add("#0D8C39");
        arrayList.add("#5639A5");
        arrayList.add("#A482F4");
        arrayList.add("#4AE3D2");
        arrayList.add("#00D28E");
        arrayList.add("#FA4848");
        arrayList.add("#434882");
        arrayList.add("#730068");
        arrayList.add("#729D38");
        arrayList.add("#C6E377");
        arrayList.add("#022D3C");
        arrayList.add("#900C3F");
        arrayList.add("#FF5733");
        arrayList.add("#FFC300");
        return arrayList;
    }
    public static List<String>colorList1() {
        List<String>arrayList = new ArrayList<>();
        //FREE
        arrayList.add("#ffffff");
        arrayList.add("#f6e58d");
        arrayList.add("#ffbe76");
        arrayList.add("#ff7979");
        arrayList.add("#badc58");
        arrayList.add("#dff9fb");
        arrayList.add("#7ed6df");
        arrayList.add("#e056fd");
        arrayList.add("#686de0");
        arrayList.add("#30336b");
        arrayList.add("#95afc0");

        arrayList.add("#00a8ff");
        arrayList.add("#9c88ff");
        arrayList.add("#fbc531");
        arrayList.add("#4cd137");
        arrayList.add("#487eb0");
        arrayList.add("#e84118");
        arrayList.add("#f5f6fa");
        arrayList.add("#7f8fa6");
        arrayList.add("#273c75");
        arrayList.add("#353b48");

        arrayList.add("#FEA47F");
        arrayList.add("#25CCF7");
        arrayList.add("#EAB543");
        arrayList.add("#55E6C1");
        arrayList.add("#CAD3C8");
        arrayList.add("#B33771");
        arrayList.add("#3B3B98");
        arrayList.add("#FD7272");
        arrayList.add("#9AECDB");
        arrayList.add("#D6A2E8");

        arrayList.add("#cd84f1");
        arrayList.add("#ffcccc");
        arrayList.add("#ff4d4d");
        arrayList.add("#ffaf40");
        arrayList.add("#fffa65");
        arrayList.add("#32ff7e");
        arrayList.add("#7efff5");
        arrayList.add("#18dcff");
        arrayList.add("#7d5fff");
        arrayList.add("#4b4b4b");

        arrayList.add("#40407a");
        arrayList.add("#706fd3");
        arrayList.add("#f7f1e3");
        arrayList.add("#34ace0");
        arrayList.add("#33d9b2");
        arrayList.add("#ff5252");
        arrayList.add("#ff793f");
        arrayList.add("#d1ccc0");
        arrayList.add("#ffb142");
        arrayList.add("#ffda79");

        arrayList.add("#FFC312");
        arrayList.add("#C4E538");
        arrayList.add("#12CBC4");
        arrayList.add("#FDA7DF");
        arrayList.add("#ED4C67");
        arrayList.add("#EE5A24");
        arrayList.add("#009432");
        arrayList.add("#0652DD");
        arrayList.add("#9980FA");
        arrayList.add("#833471");
        return arrayList;
    }
    public static List<String>colorList2() {
        List<String>arrayList = new ArrayList<>();
        //FREE
        arrayList.add("#f8f8f8");
        arrayList.add("#f9ca24");
        arrayList.add("#f0932b");
        arrayList.add("#eb4d4b");
        arrayList.add("#6ab04c");
        arrayList.add("#c7ecee");
        arrayList.add("#22a6b3");
        arrayList.add("#be2edd");
        arrayList.add("#4834d4");
        arrayList.add("#130f40");
        arrayList.add("#535c68");

        arrayList.add("#0097e6");
        arrayList.add("#8c7ae6");
        arrayList.add("#e1b12c");
        arrayList.add("#44bd32");
        arrayList.add("#40739e");
        arrayList.add("#c23616");
        arrayList.add("#dcdde1");
        arrayList.add("#718093");
        arrayList.add("#192a56");
        arrayList.add("#2f3640");

        arrayList.add("#F97F51");
        arrayList.add("#1B9CFC");
        arrayList.add("#F8EFBA");
        arrayList.add("#58B19F");
        arrayList.add("#2C3A47");
        arrayList.add("#6D214F");
        arrayList.add("#182C61");
        arrayList.add("#FC427B");
        arrayList.add("#BDC581");
        arrayList.add("#82589F");

        arrayList.add("#c56cf0");
        arrayList.add("#ffb8b8");
        arrayList.add("#ff3838");
        arrayList.add("#ff9f1a");
        arrayList.add("#fff200");
        arrayList.add("#3ae374");
        arrayList.add("#67e6dc");
        arrayList.add("#17c0eb");
        arrayList.add("#7158e2");
        arrayList.add("#3d3d3d");

        arrayList.add("#2c2c54");
        arrayList.add("#474787");
        arrayList.add("#aaa69d");
        arrayList.add("#227093");
        arrayList.add("#218c74");
        arrayList.add("#b33939");
        arrayList.add("#cd6133");
        arrayList.add("#84817a");
        arrayList.add("#cc8e35");
        arrayList.add("#ccae62");

        arrayList.add("#F79F1F");
        arrayList.add("#A3CB38");
        arrayList.add("#1289A7");
        arrayList.add("#D980FA");
        arrayList.add("#B53471");
        arrayList.add("#EA2027");
        arrayList.add("#006266");
        arrayList.add("#1B1464");
        arrayList.add("#5758BB");
        arrayList.add("#6F1E51");
        return arrayList;
    }

}
